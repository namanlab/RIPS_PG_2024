...
# Initialize delta sequence and dataframe
delta <- seq(0, 0.5, by = 0.01)
df <- tibble(delta = delta)

# Placeholder for the MSE, Bias, and Variance of each method
smooth_elastic_mse <- smooth_elastic_bias <- smooth_elastic_variance <- NULL
power_mse <- power_bias <- power_variance <- NULL
normalized_power_mse <- normalized_power_bias <- normalized_power_variance <- NULL
commensurate_power_mse <- commensurate_power_bias <- commensurate_power_variance <- NULL
```
...
for (i in delta) {
  sim <- run_simulation(nt, nc, nh, sigc, sigt, sigh, uc, ut = 1, uh = 1 + i, H = 1, N = 10000, R = 100, cutoff = 0.95)
  smooth_elastic_mse <- c(smooth_elastic_mse, sim$mse_point_est)
  smooth_elastic_bias <- c(smooth_elastic_bias, sim$bias_point_est)
  smooth_elastic_variance <- c(smooth_elastic_variance, sim$var_point_est)
}
```
...
for (i in delta) {
  sim <- run_simulation(nt, nc, nh, sigc, sigt, sigh, uc, ut = 1, uh = 1 + i, H = 1, N = 10000, R = 100, cutoff = 0.95)
  normalized_power_mse <- c(normalized_power_mse, sim$mse_point_est)
  normalized_power_bias <- c(normalized_power_bias, sim$bias_point_est)
  normalized_power_variance <- c(normalized_power_variance, sim$var_point_est)
}
...
for (i in delta) {
  sim <- run_simulation(nt, nc, nh, sigc, sigt, sigh, uc, ut = 1, uh = 1 + i, H = 1, N = 10000, R = 100, cutoff = 0.95)
  commensurate_power_mse <- c(commensurate_power_mse, sim$mse_point_est)
  commensurate_power_bias <- c(commensurate_power_bias, sim$bias_point_est)
  commensurate_power_variance <- c(commensurate_power_variance, sim$var_point_est)
}
