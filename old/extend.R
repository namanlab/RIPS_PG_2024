...
# Calculate gt for each historical dataset and then jointly generate the final posterior samples
get_control_prost <- function(params, xc, xh_list, nc, nh, H, N){
  gt_values <- numeric(length(xh_list))
  for (i in 1:length(xh_list)) {
    xh <- xh_list[[i]]
    param <- params[[i]]
    
    # Calculate gt for the current historical dataset
    sp <- ((nh[i]-1)*var(xh) + (nc-1)*var(xc))/(nh[i] + nc - 2) # pooled variance
    cong_measure <- abs(mean(xh)-mean(xc))/(sqrt(sp/nh[i] + sp/nc))
    gt_values[i] <- 1/(1 + exp(param$a + param$b*(log(cong_measure))^param$c))
    if(gt_values[i] == 0) gt_values[i] <- 0.00001 # numerical stability
  }
  # Use the gt values to jointly generate the final posterior samples
  temp <- sample_poster(xh_list, nh, xc, nc, gt_values, sim = N + 10000, nburn = 10000)
  ess <- sum(gt_values * nh) + nc
  
  return(list(prost_samples = temp$muc_post, ess = ess))
}

# Sample posterior for control arm using the calculated gt values
sample_poster <- function(xh_list, nh, xc, nc, gt_values, sim, nburn) {
  total_nh <- sum(nh * gt_values)
  total_mean_xh <- sum(sapply(1:length(xh_list), function(i) gt_values[i] * nh[i] * mean(xh_list[[i]]))) / total_nh
  total_var_xh <- sum(sapply(1:length(xh_list), function(i) gt_values[i] * (nh[i] - 1) * var(xh_list[[i]]))) / total_nh
  
  b <- 1/2 * (2 * pm[4] + (nc - 1) * var(xc) + total_var_xh + 
                total_nh * pm[2] * (total_mean_xh - pm[1])^2 / (total_nh + pm[2]))
  a <- pm[3] + (nc + total_nh) / 2
  sig <- rinvgamma(sim - nburn, a, b)
  mu <- (nc * mean(xc) + total_nh * total_mean_xh + pm[2] * pm[1]) / (nc + total_nh + pm[2])
  var <- sig / (1/nc + 1/total_nh + 1/pm[2])
  muc_post <- rnorm(sim - nburn, mu, sqrt(var)) 
