...
write.csv(final_df_1, "results/normalized_results_tau1_updated.csv")
write.csv(final_df_2, "results/normalized_results_tau2_updated.csv")
  ...
  for (i in delta1){
    ...
    for (j in delta2){
      ...
      if (nrow(final_df) %% 110 == 0) {
        write.csv(final_df, file = paste0("results/normalized_checkpoint_fc_nc_", nrow(final_df), ".csv"))
      }
    }
  }
}
# 
write.csv(final_df, "results/normalized_results_nc_fc_temp.csv")
